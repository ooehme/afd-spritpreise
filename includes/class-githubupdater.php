...
final class GithubUpdater
{
    ...
    public function plugin_information(mixed $result, string $action, object $args): mixed
    {
        ...
        return (object) [
            ...
            'homepage' => 'https://github.com/' . $this->owner . '/' . $this->repository,
            'download_link' => $release['package'],
            'requires_php' => '8.1',
            'sections' => ['description' => esc_html__('Regionale Kraftstoffpreise und konfigurierbare Szenariorechnung.', 'afd-spritpreise'), 'changelog' => wp_kses_post(nl2br($release['notes']))],
        ];
    }
    ...
    public function normalize_source(string|\WP_Error $source, string $remoteSource, \WP_Upgrader $upgrader, array $hookExtra): string|\WP_Error
    {
        ...
        if (!$wp_filesystem->move($source, $target, true)) {
            return new \WP_Error('afdsp_update_move', __('Das GitHub-Release konnte nicht in das Plugin-Verzeichnis verschoben werden.', 'afd-spritpreise'));
        }
    ...
    private function release(): ?array
    {
        ...
        $response = wp_remote_get('https://api.github.com/repos/' . rawurlencode($this->owner) . '/' . rawurlencode($this->repository) . '/releases/latest', [
            'timeout' => 10,
        ...
        foreach ((array) ($body['assets'] ?? []) as $asset) {
            if (($asset['name'] ?? '') === $this->repository . '.zip' && !empty($asset['browser_download_url'])) {
        ...
        $release = ['available' => true, 'version' => $version, 'package' => esc_url_raw($package), 'notes' => sanitize_textarea_field((string) ($body['body'] ?? ''))];
