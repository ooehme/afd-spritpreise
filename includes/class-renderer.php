...
final class Renderer
{
    ...
    private function render_full(array $config): string
    {
        ...
        $selected = $results[$config['defaultFuel']] ?? null;

        ob_start();
        ?>
        <section id="<?php echo esc_attr($id); ?>" class="afdsp afdsp--full" data-afdsp-tabs>
            <?php if ($config['showTitle']) : ?>
                <header class="afdsp-header">
                    <p class="afdsp-eyebrow"><?php esc_html_e('Regionale Kraftstoffpreise', 'afd-spritpreise'); ?></p>
                    <h2 class="afdsp-title"><?php esc_html_e('Das kostet Kraftstoff mit der AfD', 'afd-spritpreise'); ?></h2>
                    <?php if ($selected) : ?>
                        <p class="afdsp-intro"><?php echo esc_html(sprintf(__('Median aus %1$d Tankstellen im Gebiet %2$s.', 'afd-spritpreise'), $selected['station_count'], $config['areaLabel'])); ?></p>
                    <?php else : ?>
                        <p class="afdsp-intro"><?php echo esc_html($config['areaLabel']); ?></p>
                    <?php endif; ?>
                </header>
            <?php endif; ?>
            ...
            <div class="afdsp-tabs" role="tablist" aria-label="<?php esc_attr_e('Kraftstoffart', 'afd-spritpreise'); ?>">
                <?php foreach (self::LABELS as $fuel => $label) : ?>
                    <button class="afdsp-tab<?php echo $fuel === $config['defaultFuel'] ? ' is-active' : ''; ?>" type="button" role="tab" id="<?php echo esc_attr($id . '-tab-' . $fuel); ?>" aria-controls="<?php echo esc_attr($id . '-panel-' . $fuel); ?>" aria-selected="<?php echo $fuel === $config['defaultFuel'] ? 'true' : 'false'; ?>" data-afdsp-tab="<?php echo esc_attr($fuel); ?>"><?php echo esc_html($label); ?></button>
                <?php endforeach; ?>
            </div>

            <?php foreach (self::LABELS as $fuel => $label) : ?>
                <div class="afdsp-panel" id="<?php echo esc_attr($id . '-panel-' . $fuel); ?>" role="tabpanel" aria-labelledby="<?php echo esc_attr($id . '-tab-' . $fuel); ?>" data-afdsp-panel="<?php echo esc_attr($fuel); ?>"<?php echo $fuel !== $config['defaultFuel'] ? ' hidden' : ''; ?>>
                    <?php if (!$results[$fuel]) : ?>
    ...
    private function price_board(array $result, array $config): string
    {
        ...
        <div class="afdsp-board">
            ...
            <div class="afdsp-price-grid">
                <div class="afdsp-price"><span><?php esc_html_e('Aktueller Kraftstoffpreis', 'afd-spritpreise'); ?></span><strong><?php echo esc_html($this->price($calc['current_gross'])); ?></strong></div>
                <div class="afdsp-price afdsp-price--scenario"><span><?php esc_html_e('Nach AfD-Forderungen', 'afd-spritpreise'); ?></span><strong><?php echo esc_html($this->price($calc['scenario_gross'])); ?></strong></div>
            ...
            <div class="afdsp-facts">
                <?php if ($config['showTankSaving']) : ?><div><span><?php esc_html_e('Bei 50 Litern', 'afd-spritpreise'); ?></span><strong><?php echo esc_html($this->money($calc['saving_50l']) . ' weniger'); ?></strong></div><?php endif; ?>
                <?php if ($config['showCheapestStation']) : ?>
                    <div class="afdsp-station"><span><?php esc_html_e('Günstigste Tankstelle', 'afd-spritpreise'); ?></span><strong><?php echo esc_html($station['name']); ?></strong><small><?php echo esc_html(trim($station['street'] . ' · ' . $station['postcode'] . ' ' . $station['city'], ' ·')); ?></small><b><?php echo esc_html($this->price($station['price'])); ?></b><?php if ($station['priceTs']) : ?><time datetime="<?php echo esc_attr($station['priceTs']); ?>"><?php echo esc_html($this->date($station['priceTs'])); ?></time><?php endif; ?></div>
                <?php endif; ?>
            </div>
        </div>
        <?php
    ...
    private function demands(array $result): string
    {
        ...
        <section class="afdsp-receipt" aria-label="<?php esc_attr_e('Forderungen und Quellen', 'afd-spritpreise'); ?>">
            <h3><?php esc_html_e('In der Rechnung', 'afd-spritpreise'); ?></h3>
            ...
            <?php echo $this->demand_row(__('Mehrwertsteuer auf Kraftstoffe auf 7 % senken', 'afd-spritpreise'), $this->number($c['vat_current'], 0) . ' % → ' . $this->number($c['vat_scenario'], 0) . ' %', __('auf den verbleibenden Nettopreis', 'afd-spritpreise'), 'BT-Drs. 21/5326', 'https://dserver.bundestag.de/btd/21/053/2105326.pdf'); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
    ...
    {
        return '<article class="afdsp-demand"><h4>' . esc_html($title) . '</h4><p><strong>' . esc_html($change) . '</strong><span>' . esc_html($effect) . '</span></p><a href="' . esc_url($url) . '" rel="external noopener" target="_blank">' . esc_html($source) . '</a></article>';
    }

    private function method(array $result, string $areaLabel): string
    {
        $text = sprintf(
            __('Median aus %1$d gültigen, aktiven Tankstellen im Gebiet %2$s. Der aktuelle Bruttopreis wird zunächst um die aktuelle Mehrwertsteuer bereinigt. Danach werden aktuelle Energie- und CO₂-Kosten entfernt, die Szenariowerte addiert und die Szenario-Mehrwertsteuer aufgeschlagen. Intern wird ohne vorzeitige Rundung gerechnet.', 'afd-spritpreise'),
            $result['station_count'],
            $areaLabel
        );
        return '<section class="afdsp-method"><h3>' . esc_html__('Methodik', 'afd-spritpreise') . '</h3><p>' . esc_html($text) . '</p><p class="afdsp-source">' . wp_kses_post(sprintf(__('Preisdaten: %s · MTS-K', 'afd-spritpreise'), '<a href="https://tankpuls.de/" rel="external nofollow">TankPuls</a>')) . '</p></section>';
    }
